...
rD <- rsDriver(browser = c("firefox"), port = 4494L)
driver <- rD[["client"]]
...
# const_name_2 <- driver$findElement("css", '.ge-result__name')$getElementText() # alternative way speifying CSS selector differently
...
results_table  # to see the table
```
...
get_results_by_const <- function(const_name, sec = 4){
  ...
  search_field <- ____________________________________________________
  ...
  while(driver$findElement(using = 'class name', 
                           value = 'ge-lookup__suggestions')$getElementText() %>% 
        nchar() == 0) {
    ___________(1)  # Hint: Sleep for 1 second
  }
  ...
  party_class <- results_html %>% 
    html_nodes(xpath = "______________") %>%
    html_attr("____________")
