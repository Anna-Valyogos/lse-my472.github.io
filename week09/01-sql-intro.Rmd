...
```{r, eval=FALSE}
fls <- list.files("facebook-data/posts", full.names = TRUE) # creates character vector of file names
...
for (f in fls){
  ...
  dbWriteTable(db, "posts", fb, append = TRUE) # append creates table fb if it does not already exist, and appends to it if it does
...
```{r}
db <- dbConnect(RSQLite::SQLite(), "facebook-data/facebook-db.sqlite") # connect to existing database we have just created
