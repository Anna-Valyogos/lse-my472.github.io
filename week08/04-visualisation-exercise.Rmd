...
unemp$date <- ymd(unemp$date)

# Create a line-plot with unemployment and date
p <- ggplot(unemp, aes(x = date, y = unemp))
p + geom_line() +
  
  # Fix axes and add colored rectangles
  ylab("Unemployment Rate") +
  # scale_x_datetime(date_breaks = c(1960, 1980, 2000))
  # geom_rect(data = pres)
        


# Adding president names
  geom_text(data = pres, mapping = aes(x = start, y = 0.10, label = name),
            angle = 90, vjust = - 0.2, hjust = 0.25) + #wrong side of the line now



# Adding lines to separate presidencies
  geom_vline(data = pres, xintercept = start, linetype = 5) +
