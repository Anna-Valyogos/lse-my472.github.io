...
```{r}
authentication <- list(consumer_key = "Z5IKectiIqZcq9cJVXvJMQ5ND",
                 consumer_secret = "mCliCAGhuMB4pLNbICafDMqTv9Ntmv7XuuBkJmgkLljSrRtpRL",
                 access_token = "1320776729556885504-NlaryhgxxYaJW1KCzXVJHAoppuJpxR",
                 access_token_secret = "i7unrCU1FSn1jKD33vRKhqLeCqBTxrms3xmMJ9u4ZzDdi")
```
...
```{r}
sample_tweets <- stream_tweets(q = "", timeout = 30)
head(sample_tweets)
...
class(sample_tweets)
sample_tweets
# already a neat data frame - thanks to Pablo who built this package
```
...
```{r}
ht <- str_extract_all(sample_tweets$text, "#[A-Za-z0-9_]+") # regex looks for a hash symbol and whatever capital letter, whatever smaller case letter and whatever number, underscore comes after it (in any amount) in a sequence - in any order, and + means in any length 
ht <- unlist(ht)
...
# useful especially if you collect a lot of data 
```
...
```{r}
stream_tweets(q = "", timeout = 10, parse = FALSE, file_name = "collected_tweets.json")
```
...
```{r}
uid_tweets <- stream_tweets(q = "25073877", timeout = 5) # Trump's account ID as the search word, so all tweets that mention Donald Trump in the last 5 secs will be collected
...
```{r}
geo_tweets <- stream_tweets(q = c(125, 26 , -65, 49), timeout = 30) # all tweets coming from the US that are geolocated
...
states <- map.where(database = "state", x = geo_tweets$long, y = geo_tweets$lat)
...
ggplot(map.data) + geom_map(aes(map_id = region), map = map.data, fill = "grey90", 
    ...
    geom_point(data = geo_tweets, aes(x = lng, y = lat), size = 1,
               alpha = 1/5, color = "red") +
