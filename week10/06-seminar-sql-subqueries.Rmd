...
dbListTables(db) # check if it worked
```

1. What are the screen names of the top 3 posters?
```{r}
dbGetQuery(db,
           "SELECT screen_name, COUNT(*) AS num_of_posts
           FROM posts
...
# make this into data table
library(DT)
dbGetQuery(db,
           "SELECT *
           FROM posts
           WHERE screen_name IN ('CongressmanNadler', 'HurdOnTheHill', 'RepMikeThompson')") %>% datatable()
```
