...
db <- dbConnect(RSQLite::SQLite(), "./facebook-data/facebook-db-mini.sqlite")
# reading the first file
congress <- read.csv("./facebook-data/congress-facebook-2017.csv",
	stringsAsFactors=F)
...
fls <- list.files("./facebook-data/posts", full.names=TRUE)
set.seed(20191212)
...
dbListTables(db)
dbGetQuery(db, 
           "SELECT COUNT(*) as number_of_rows FROM congress")
dbGetQuery(db, 
           "SELECT COUNT(*) as number_of_rows FROM posts")
```
...
dbGetQuery(db,
           "SELECT COUNT(*)
           FROM posts AS p JOIN congress AS c
              ON p.screen_name = c.screen_name")

# by default this is an inner-join
# it tells us that xxx record overlap in the two tables

```

3. How many members of congress do not have any posts?
```{r}
# left exclusive join for congress
# (congress is left, posts are right, and intersection is not included)
dbGetQuery(db,
           "SELECT COUNT(*)
...
# congress left, posts right
dbGetQuery(db,
           "SELECT COUNT(DISTINCT posts.screen_name)
           FROM congress RIGHT JOIN posts
              ON congress.screen_name = posts.screen_name
           WHERE congress.screen_name IS NULL")
# RIGHT JOIN not supported, so need to rewrite the query
