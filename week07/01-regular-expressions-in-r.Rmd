...
```{r}
current_dir <- "/Users/valyo/Downloads/MASTERS/MY472/lse-my472.github_all_material/week07/"
```

Load the txt file into a character vector with `read_file`:

```{r}
sample_text <- read_file(paste(current_dir, "practice_regular_expressions.txt", sep = ""))
...
# To allow for characters such as \ in the regular expression
# (e.g. \d or \w) to be understood by R, \ has to be escaped 
# i.e. handled as a simple string character, not as a special
# character. Therefore the first \ is the escape operation by R,
# the sencond is part of the regular expression that we are searching
# as a string.

# regular expression we search for in Atom:
# \d{1,2}[:.\s-]?\d{0,2}
# for R to treat this properly we ought to escape the \ characters
# to be treated as normal characters, so the R code becomes:

str_extract(sample_text, "\\d{1,2}[:.\\s-]?\\d{0,2}")
```

```{r}
str_extract_all(sample_text, "\\d{1,2}[:.\\s-]?\\d{0,2}") # returns a list
```
